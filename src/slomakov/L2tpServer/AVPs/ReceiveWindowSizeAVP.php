...
class ReceiveWindowSizeAVP extends BaseAVP
{
    ...
    public static function import($data)
    {
        ...
        if ($avp->length != 8) {
            throw new IgnoreAVPException("Invalid length for Receive Window Size AVP");
        }
    ...
    {
        if (!$this->isMandatory) {
            throw new IgnoreAVPException("Receive Window Size AVP should not be mandatory.");
        }
        if ($this->isHidden) {
            throw new IgnoreAVPException("Receive Window Size AVP should not be hidden.");
        }
    }
    ...
    {
        return AvpType::RECEIVE_WINDOW_SIZE_AVP;
    }
